<template>
  <div v-if="AuthService.isAdmin(auth0)"
    ...
    <div :class="{ invisible: !state.isEditMode }">
      <input type="checkbox" id="show-preview" @change="onPreviewChangeAsync" name="showPreview"
    ...
    <div v-if="AuthService.isAdmin(auth0) && state.isEditMode">
      ...
      <CodeEditor v-model="state.content" language="html" v-on:update:model-value="onContentChangeAsync" :height="40">
      </CodeEditor>
      <div v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-xl border-slate-400 rounded border-2 p-1 pl-2 my-2" v-html="state.templatedContent"></div>
