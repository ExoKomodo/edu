<template>
  <div v-if="AuthService.isAdmin(auth0)"
    ...
    <span class="inline-block" :class="{ invisible: !state.isEditMode }">
      <Button :handler="() => handler(state)" :text="handlerText"></Button>
    </span>
    ...
    <div v-if="AuthService.isAdmin(auth0) && state.isEditMode">
      ...
      <p v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-2xl font-bold border-slate-400 rounded border-2 p-1 pl-2">{{ state.name?.toUpperCase() }}</p>
      ...
      <p v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-xl border-slate-400 rounded border-2 p-1 pl-2 my-2" v-html="state.description"></p>
      <div class="text-white border-white border-2 rounded p-1 pl-2 my-2 mr-8">Difficulty (integer)</div>
      <CodeEditor v-model="state.difficulty" :height="2"></CodeEditor>
      <div v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-xl border-slate-400 rounded border-2 p-1 pl-2 my-2" v-html="state.difficulty"></div>
      <div class="text-white border-white border-2 rounded p-1 pl-2 my-2 mr-8">Content</div>
      <CodeEditor v-model="state.content" language="html" :height="40"></CodeEditor>
      <div v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-xl border-slate-400 rounded border-2 p-1 pl-2 my-2" v-html="state.content"></div>
      <div class="text-white border-white border-2 rounded p-1 pl-2 my-2 mr-8">Difficulty (integer)</div>
      <CodeEditor v-model="state.difficulty" :height="2"></CodeEditor>
      <div v-if="AuthService.isAdmin(auth0) && state.isEditMode && state.showPreview"
        class="text-xl border-slate-400 rounded border-2 p-1 pl-2 my-2" v-html="state.difficulty"></div>
