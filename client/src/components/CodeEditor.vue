<template>
  <label class="mr-2">Language:
    <select @change="onLanguage" v-model="selected.language" class="text-black">
      <option v-for="option in languageOptions" v-bind:value="option.id" :selected="selected.language == option.id">
        {{ option.value }}</option>
    </select>
  </label>
  <label>Theme:
    <select @change="onTheme" v-model="selected.theme" class="text-black">
      <option v-for="option in themeOptions" :value="option.id" :selected="selected.theme == option.id">{{ option.value
        }}
      </option>
    </select>
  </label>
  <v-ace-editor ref="codeEditor" :value="selected.content" @change="onContent" :lang="selected.language"
    :theme="selected.theme" :style="`height: ${height}rem`" :options="{
      useWorker: true,
