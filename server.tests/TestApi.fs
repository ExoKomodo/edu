...
let unauthenticatedAuth0HttpClient =
    new HttpClient(BaseAddress = new Uri("https://exokomodo.us.auth0.com"))

let serializer = Json.Serializer()

type TestDependencies() =
    static let server = runTestApi ()
    ...
    static let clientParams =
        Auth0ClientParams.FromEduClientCredentials clientId clientSecret

    static let accessToken =
        match
            getMachineToMachineAccessTokenAsync
                unauthenticatedAuth0HttpClient
                serializer
                clientParams
