...
[<CLIMutable>]
type BlobQueryArgs = { url : string }

[<CLIMutable>]
type BlogMetadata = { Description : string; Title : string }
...
type Dependencies = {
    ...
    static member InitializeMongo () =
        ...
        | _ ->
            ...
            | null ->
                printfn
                    "You must set your 'MONGODB_DATABASE' environment variable."
    ...
    static member TestS3Connection (client : AmazonS3Client) : bool =
        let buckets =
            client.ListBucketsAsync(new CancellationToken())
            |> Async.AwaitTask
            |> Async.RunSynchronously
        // NOTE: 200 <= status < 300
        HttpStatusCode.OK <= buckets.HttpStatusCode
        && buckets.HttpStatusCode < HttpStatusCode.Ambiguous

    static member ConnectToS3 () =
        printfn "Connecting to S3..."
        let config = new AmazonS3Config(ServiceURL = s3Endpoint)
    ...
    static member GenerateUpdateCourse (course : Course) =
        let mutable update =
            Builders<Course>.Update
                .Set((fun _course -> _course.Content), course.Content)

        update <- update.Set((fun _course -> _course.Metadata), course.Metadata)
        update
