...
let paidEmailUsers = [ "jamesaorson@gmail.com" ]

let paidAppleSubs = [ "000029.21872332bd244cca8b5cf65af46d5e00.2035" ] // jamesaorson@gmail.com

let paidAuth0Subs = [ "645e74e43d813747ca2578ee" ] // exokomodo@gmail.com

let paidGoogleSubs = [
    "107263294567498975062" // exokomodo@gmail.com
    "113937910186417093745"
] // jamesaorson@gmail.com

let paidGithubSubs = [ "17893076" ] // jamesaorson@gmail.com

let paidMicrosoftSubs = [ "9e29fe3ca9a421c1" ] // jamesaorson@gmail.com
        ...
        let result =
            ...
            | Some authorizationHeader ->
                ...
                | StringPrefix "Bearer " token ->
                    auth0HttpClient.DefaultRequestHeaders.Authorization <-
                    ...
                    | Some User ->
                        match
                            getUserInfoAsync
                                auth0HttpClient
                                (ctx.GetJsonSerializer() :?> Json.Serializer)
                            ...
                            | Some email ->
                                ...
                                if isAllowed then
                                    ...
                                    RequestErrors.FORBIDDEN
                                        "Not an admin or paid user, logging in via email"
                            | None ->
                                let subResults =
                                    match userInfo.Sub with
                                    | StringPrefix "apple|" subId ->
                                        Some(subId, paidAppleSubs)
                                    | StringPrefix "auth0|" subId ->
                                        Some(subId, paidAuth0Subs)
                                    | StringPrefix "github|" subId ->
                                        Some(subId, paidGithubSubs)
                                    | StringPrefix "google-oauth2|" subId ->
                                        Some(subId, paidGoogleSubs)
                                    | StringPrefix "windowslive|" subId ->
                                        Some(subId, paidMicrosoftSubs)
                                    | _ -> None
                                ...
                                | None ->
                                    RequestErrors.FORBIDDEN
                                        $"Not a supported auth method with sub={userInfo.Sub}"
                | _ -> notLoggedIn
